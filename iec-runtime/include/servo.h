...
#define SV_TASK_PRIORITY 97			/* 伺服任务优先级 */
...
#define SERVO_RUN    1			/* 启动运行，伺服上电 */
#define SERVO_STOP	 0			/* 停止伺服数据发送 */
...
} AXIS_STATUS_AREA; /* 每个轴对应伺服从站信息 */

typedef struct {
	AXIS_CTRL_AREA ctrl_area;								/* 伺服主控制域 */
	AXIS_STATUS_AREA status_area[MAX_AXIS_COUNT];			/* 伺服从站状态域 */
	uint8_t axis_count; 									/* 轴个数 */
	uint8_t data_status;									/* 插补数据是否为过时数据，0：过时； 1：未过时 */
