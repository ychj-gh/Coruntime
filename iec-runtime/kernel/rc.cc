...
void rc_shm_servo_write(RCMem *rc_shm, RobotAxisActualInfo *axis_actual_info){
    rt_mutex_acquire(&rc_mutex_desc, TM_INFINITE);
    for(int i =0; i < ROBOT_AXIS_COUNT; i ++){
        rc_shm->actual_info.axis_info[i].actual_pos = axis_actual_info->axis_info[i].actual_pos;
    }
    rt_mutex_release(&rc_mutex_desc);
}
...
void rc_shm_servo_read(RCMem *rc_shm, RobotInterpData *axis_command_info){
    ...
    for(int i = 0; i < ROBOT_AXIS_COUNT; i ++) {
        axis_command_info->interp_value[i].command_pos = rc_shm->interp_data.interp_value[i].command_pos ;
        axis_command_info->interp_value[i].command_vel = rc_shm->interp_data.interp_value[i].command_vel ;
        axis_command_info->interp_value[i].command_acc = rc_shm->interp_data.interp_value[i].command_acc ;
        // fprintf(stderr, "axis %d : %lf\n", i, axis_command_info->interp_value[i].command_pos);
        // printf("\n");
    }
